...
from .content_manager_controller import (count_table_info, fetch_state,
                                         ...
                                         reroute_page, save_database_status)
...
@content_manager_bp.route('/content-manager/database-content', methods=['GET', 'POST', 'PUT', 'DELETE'])
@login_required
...
@content_manager_bp.route('/content-manager/save', methods=['POST'])
@login_required
def save_database():
    json_data = save_database_status()

    current_app.logger.info(f"Result dict from content_manager_bp.save_database, result: {json_data['result']}, msg: {json_data['msg']}")
    return jsonify(json_data)